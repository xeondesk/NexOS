...
const routes = {
  GET: {
    ...
    '/history': (req, res) => {
      ...
      // isAdmin is only honored for effective admins (loopback, or remote
      // clients that presented the token).
      const isAdmin =
        req.nexosAdmin === true &&
        parsedUrl.searchParams.get('isAdmin') === 'true'
...
const server = http.createServer(async (req, res) => {
  ...
  // Docker port publishing). When NEXOS_LOG_PROXY_TOKEN is set, remote
  // clients must present it (Authorization: Bearer <token> or ?token=).
  const { isLocalhost, allowRemote, tokenAuthed } = classifyRequest(req)

  if (!isLocalhost && !allowRemote) {
    return jsonResponse(res, 403, { error: 'Forbidden' })
  }
  if (!isLocalhost && process.env.NEXOS_LOG_PROXY_TOKEN && !tokenAuthed) {
    return jsonResponse(res, 401, { error: 'Unauthorized' })
  }

  // Effective admin: loopback clients are trusted (isAdmin stays client-
  // asserted); remote clients are non-admin unless they presented the token.
  req.nexosAdmin = isLocalhost || tokenAuthed
...
// Classify a request: loopback origin, whether remote access is opted in, and
// whether the client presented a valid NEXOS_LOG_PROXY_TOKEN (header or
// ?token= query param for browser WebSocket clients).
function classifyRequest(req) {
  ...
  const token = process.env.NEXOS_LOG_PROXY_TOKEN
  let tokenAuthed = false
  if (token) {
    const header = req.headers['authorization'] || ''
    const match = /^Bearer\s+(.+)$/i.exec(header)
    tokenAuthed = !!(match && match[1] === token)
    if (!tokenAuthed && req.url) {
      try {
        const parsedUrl = new URL(req.url, 'http://localhost')
        tokenAuthed = parsedUrl.searchParams.get('token') === token
      } catch {}
    }
  }
  return { isLocalhost, allowRemote, tokenAuthed }
}
...
wss.on('connection', (ws, req) => {
  ...
  if (!isLocalhost) {
    ...
    if (process.env.NEXOS_LOG_PROXY_TOKEN && !tokenAuthed) {
      ws.close(1008, 'Unauthorized')
      return
    }
  }

  // Remote clients are non-admin unless they presented the token.
  const isAdmin =
    (isLocalhost || tokenAuthed) &&
    parsedUrl.searchParams.get('isAdmin') === 'true'
